...
BEGIN
    ...
    LOCK TABLE MVCD_INVENTARIO IN ROW EXCLUSIVE MODE;
    IF (MVCD_COMPROBAR_INVENTARIO) THEN
...
BEGIN
    LOCK TABLE MVCD_CABECERA_VENTA IN ROW EXCLUSIVE MODE;
    UPDATE MVCD_CABECERA_VENTA 
        SET TOTAL_VENTA = MVCD_APLICAR_PROMOCION_F(ID_PROMOCION_F,ID_CABECERA_VENTA_F)
    WHERE ID_CABECERA_VENTA=ID_CABECERA_VENTA_F;
    COMMIT;
    EXCEPTION
    WHEN PROGRAM_ERROR THEN
...
BEGIN
    OPEN p_cursor FOR
        SELECT SUM(C.CANTIDAD*C.ID_PRODUCTO),P.NOMBRE_PRODUCTO 
