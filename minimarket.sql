...
IS
    Valor_P boolean := MVCD_COMPROBAR_INVENTARIO(ID_PRODUCTO_P,CANTIDAD_VENTA_P);
BEGIN
    LOCK TABLE MVCD_CUERPO_VENTA IN ROW EXCLUSIVE MODE;
    LOCK TABLE MVCD_INVENTARIO IN ROW EXCLUSIVE MODE;
    IF (Valor_P = TRUE) THEN
...
BEGIN
    OPEN p_cursor FOR
        ...
        WHERE H.FECHA_VENTA BETWEEN ADD_MONTHS(TRUNC(SYSDATE), -1) AND TRUNC(SYSDATE) +1
