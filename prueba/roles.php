...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'insertar') {
        ...
        $stmt = $conn->prepare("BEGIN MVCD_C_ROLES(:area_trabajo); END;");
        $stmt->bindParam(':area_trabajo', $area_trabajo);
        $stmt->execute();
        echo "Registro insertado exitosamente.";
    } elseif ($action === 'listar') {
        $stmt = $conn->query("SELECT ID_ROL, AREA_TRABAJO FROM MVCD_ROLES");
        ...
        $ID_ROL = $_POST['id_rol'];
        $stmt = $conn->prepare("BEGIN MVCD_D_ROLES(:ID_ROL); END;");
        ...
        $ID_ROL = $_POST['id_rol'];
        $area_trabajo = $_POST['area_trabajo'];
        $stmt = $conn->prepare("BEGIN MVCD_U_ROLES(:ID_ROL,:area_trabajo); END;");
        $stmt->bindParam(':ID_ROL', $ID_ROL);
        $stmt->bindParam(':area_trabajo', $area_trabajo);
        $stmt->execute();
