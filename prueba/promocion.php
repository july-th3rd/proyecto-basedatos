...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'insertar') {
        ...
        $stmt = $conn->query("SELECT ID_PROMOCION, DESCUENTO FROM MVCD_PROMOCIONES");
        echo "<table><tr><th>ID</th><th>Descuento</th></tr>";
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr><td>{$row['ID_PROMOCION']}</td><td>{$row['DESCUENTO']}</td></tr>";
        }
        echo "</table>";
